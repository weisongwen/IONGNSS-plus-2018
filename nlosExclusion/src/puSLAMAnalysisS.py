...
class SLAMAnalysis(QMainWindow): # by timer   For paper NLOS exclusion caused by double-decekr bus

    def __init__(self, parent=None):
        QMainWindow.__init__(self, parent)
        self.setWindowTitle('evaluation')
        rospy.Subscriber('/odom_CPT', Odometry, self.CallCPTOdom) # subscribe to the
        # rospy.Subscriber('/poseCarto', Odometry, self.CallCartoPose)  # subscribe to the
        rospy.Subscriber('/optimizedOdom', Odometry, self.CallCartoPose)  # subscribe to the
        self.CPTOdom_ = Odometry()
        self.cartoPose = Odometry()
        self.cartoPoseIndex_ =[]
        self.cartoPose_ =[]

        self.GPSWeekSec_ = 0.0
        self.GPSWeekSecLis = []
        self.totaError = [] # total error
        self.totErrorMea = []
        self.totErrorStd = []

        self.latErr = [] # lateral error
        self.latErrMea = []
        self.latErrStd = []

        self.lonErr = [] # longitunal error
        self.lonErrMea = []
        self.lonErrStd = []
        ...
        self.on_draw()
        self.timer = QtCore.QTimer()
        QtCore.QObject.connect(self.timer, QtCore.SIGNAL("timeout()"), self.SLAMErrorEvaluation)
    ...
    def CallCPTOdom(self,data):
        self.CPTOdom_ = data
        self.GPSWeekSec_ =self.CPTOdom_.header.seq

    def CallCartoPose(self,data):
        self.cartoPose = data
        self.cartoPose_.append(1 * math.sqrt(self.cartoPose.pose.pose.position.x * self.cartoPose.pose.pose.position.x + self.cartoPose.pose.pose.position.y * self.cartoPose.pose.pose.position.y))
        self.cartoPoseIndex_.append(len(self.cartoPose_))

    def SLAMErrorEvaluation(self):
        delta_x = self.CPTOdom_.pose.pose.position.x - self.cartoPose.pose.pose.position.x
        delta_y = self.CPTOdom_.pose.pose.position.y - self.cartoPose.pose.pose.position.y
        delta_z = self.CPTOdom_.pose.pose.position.z - self.cartoPose.pose.pose.position.z
        delta_s = math.sqrt(delta_x * delta_x + delta_y * delta_y + delta_z * delta_z)
        self.totaError.append(float(delta_s))
        self.GPSWeekSecLis.append(float(self.GPSWeekSec_))
        print 'totalMeanEr_', sum(self.totaError) / (len(self.totaError)), 'Totalstd_----', np.std(self.totaError)
        latEr,lonEr = self.calculLateralLongiErr(self.CPTOdom_,self.cartoPose)
        self.latErr.append(float(latEr))
        self.lonErr.append(float(lonEr))
        print 'latMeanEr_', sum(self.latErr) / (len(self.latErr)), 'latstd_----', np.std(self.latErr)
        print 'lonMeanEr_', sum(self.lonErr) / (len(self.lonErr)), 'lonstd_----', np.std(self.lonErr)
        self.axesCurv_1.tick_params(axis='both',labelsize = 25)
        self.axesCurv_1.plot(self.cartoPoseIndex_, self.cartoPose_, '-', linewidth='1', color='dodgerblue')
        print 'mean error',np.mean(self.cartoPose_),'error std',np.std(self.cartoPose_)
        self.PercentCal(self.cartoPose_)
        # self.axesCurv_1.plot(self.GPSWeekSecLis, self.totaError, '-', linewidth='1', color='dodgerblue', label='TotalError')
        # self.axesCurv_2.plot(self.GPSWeekSecLis, self.latErr, '-', linewidth='1', color='dodgerblue',
        #                      label='latErr')
        # self.axesCurv_3.plot(self.GPSWeekSecLis, self.lonErr, '-', linewidth='1', color='dodgerblue',
        #                      label='lonErr')
        self.axesCurv_1.legend(loc=9, ncol=5, shadow=True)
        # self.axesCurv_2.legend(loc=9, ncol=5, shadow=True)
        # self.axesCurv_3.legend(loc=9, ncol=5, shadow=True)
    ...
    def calculLateralLongiErr(self,CPT_,Carto):
        CPTPose = Odometry()
        CPTPose = CPT_
        cartoPose = Odometry()
        cartoPose = Carto
        Quaternion_ = (CPTPose.pose.pose.orientation.x,CPTPose.pose.pose.orientation.y,CPTPose.pose.pose.orientation.z,CPTPose.pose.pose.orientation.w)
        q_orig = Quaternion_
        q_rot = quaternion_from_euler(0, 0, math.pi)
        q_new = quaternion_multiply(q_rot, q_orig)
        euler_  = tf.transformations.euler_from_quaternion(q_new)
        # print 'yaw---',euler_[2]*180/(math.pi)
        vectorDir_= (math.cos(euler_[2]),math.sin(euler_[2])) # heading of vehicle
        errorVect_ = ((CPTPose.pose.pose.position.x-cartoPose.pose.pose.position.x),(CPTPose.pose.pose.position.y-cartoPose.pose.pose.position.y))
        errorSumXY = math.sqrt(errorVect_[0] * errorVect_[0] + errorVect_[1] * errorVect_[1])
        longiErr = vectorDir_[0] * errorVect_[0] + vectorDir_[1] * errorVect_[1]
        vectorDirVert = (math.cos(euler_[2] + (math.pi/2.0)), math.sin(euler_[2] + (math.pi/2.0))) # vertical heading of vehicle
        latiErr = vectorDirVert[0] * errorVect_[0] + vectorDirVert[1] * errorVect_[1]
        # print 'longiErr',longiErr,'latiErr',latiErr,'errorSumXY',errorSumXY
        return latiErr,longiErr
    ...
    def create_main_frame(self):
        ...
        self.canvas.setParent(self.main_frame)
        # self.axes = self.fig.add_subplot(111, projection='3d')
        self.axesCurv_1 = self.fig.add_subplot(111)
        # self.axesCurv_2 = self.fig.add_subplot(312)
        # self.axesCurv_3 = self.fig.add_subplot(313)
        # self.axes.get_zaxis().set_visible(False)
        self.canvas.mpl_connect('pick_event', self.on_pick)
...
if __name__ == '__main__':
    ...
    rospy.init_node('puSLAMAnalysis_', anonymous=True)
    puSLAMAnalysis=SLAMAnalysis()
    print 'process SLAM Analysis...'
    print '--------------------------'
    puSLAMAnalysis.show()
    app.exec_()